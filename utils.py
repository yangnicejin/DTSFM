...
def filter_scores_e(s, r, t, scores, all_ans_dict):
    """
    过滤实体预测任务中已知的正确答案
    """
    key = (s, r, t)
    existing_answers = all_ans_dict.get(key, set())
    filtered_scores = scores.copy()
    for ans in existing_answers:
        filtered_scores[ans] = -np.inf  # 将已知答案的得分设为最低
    return filtered_scores

def filter_scores_r(s, o, t, scores, all_ans_dict):
    """
    过滤关系预测任务中已知的正确答案
    """
    key = (s, o, t)
    ...
    filtered_scores = scores.copy()
    for ans in existing_answers:
        filtered_scores[ans] = -np.inf
    return filtered_scores

def sort_and_rank(score, target):
    _, indices = torch.sort(score, dim=1, descending=True)
    # 确保 target 和 indices 都在相同的设备上
    device = score.device  # 使用 score 的设备
    target = target.to(device)  # 将 target 移动到与 score 相同的设备
...
def get_total_rank(test_triples, score, all_ans, eval_bz, rel_predict=0):
    ...
    for idx in range(n_batch):
        ...
        # 确保 target 和 score_batch 在相同的设备上
        device = score.device  # 使用 score 的设备
        if rel_predict == 1:
            target = test_triples[batch_start:batch_end, 1].to(device)  # 确保 target 在相同设备上
        elif rel_predict == 2:
            target = test_triples[batch_start:batch_end, 0].to(device)  # 确保 target 在相同设备上
        else:
            target = test_triples[batch_start:batch_end, 2].to(device)  # 确保 target 在相同设备上
